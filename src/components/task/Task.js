...
export default function Task({
  ...
  return (
    <>
      ...
      <EditTaskModal
        show={showEditTask}
        showEditTask={handleEditTask}
        item={item}
      />
      <Modal show={deleteModal} onHide={handleCloseDelete}>
        <Modal.Body>
          <p>
            Are you sure you want to delete the task <b>{item.title}</b>?
          </p>
          <Button variant="primary me-2" onClick={handleDeleteTask}>
            Delete
          </Button>
          ...
          <div className="task position-relative">
            <div className={styles.taskInner}>
              <div className={clsx(styles.taskControls, "task-footer d-flex")}>
                <Button variant="link btn-icon me-2" onClick={handleOpenDelete}>
                  <BsTrash />
                </Button>
                <Button variant="link btn-icon" onClick={handleEditTask}>
                  <BsPencil />
                </Button>
              </div>
              ...
              {attachments && (
                <div className={styles.taskAttachments}>
                  {attachments.map((file) => (
                    <a key={uuidv4()} href={file} target="blank">
              ...
              <div className={styles.taskInfo}>
                ...
                <div className={styles.info}>
                  <Badge bg={color} className="item-status mb-2">
                    {status}
                  </Badge>
                </div>
