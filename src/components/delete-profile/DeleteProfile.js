...
export default function DeleteProfile({ show, onHide }) {
  ...
  return (
    <>
      ...
      <ModalWindow show={show} onHide={onHide} title="Profile deletion" text="Are you sure you want to delete your account?">
        <Form onSubmit={handleUserDelete}>
          <Form.Group className="mb-3" id="relogPassword">
            <Form.Control type="password" placeholder="Password" ref={relogPasswordRef} className="inputRound" />
          </Form.Group>
          <div className="buttonsContainer mt-4 mb-3">
            <Button variant="primary" type="submit" className="btnRound mx-2">Delete</Button>
            <Button variant="danger" onClick={onHide} className="btnRound mx-2">Cancel</Button>
          </div>
        </Form>
      </ModalWindow>
    </>
